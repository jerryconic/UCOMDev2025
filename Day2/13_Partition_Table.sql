...
CREATE PARTITION FUNCTION [fnPart](date) AS RANGE RIGHT FOR VALUES (N'2006-01-01', N'2007-01-01', N'2008-01-01')

CREATE PARTITION SCHEME [schPart] AS PARTITION [fnPart] TO ([YR_BEFORE], [YR_2006], [YR_2007], [YR_2008])
...
CREATE CLUSTERED INDEX [ClusteredIndex_on_schPart_638821295538358148] ON [dbo].[BigOrder]
(
	[order_date]
)WITH (SORT_IN_TEMPDB = OFF, DROP_EXISTING = OFF, ONLINE = OFF) ON [schPart]([order_date])

DROP INDEX [ClusteredIndex_on_schPart_638821295538358148] ON [dbo].[BigOrder]

COMMIT TRANSACTION

GO

INSERT INTO dbo.BigOrder(order_date)
VALUES('2005-1-1');
GO 8192
...
--Add new filegroup YR_2009(db01_2009.ndf)
USE [master]
GO
...
GO
ALTER DATABASE [db01] ADD FILE ( NAME = N'db01_2009', FILENAME = N'C:\SQLData\db01_2009.ndf' , SIZE = 8192KB , FILEGROWTH = 4096KB ) TO FILEGROUP [YR_2009]
GO
...
ALTER PARTITION FUNCTION fnPart()
MERGE RANGE('2006-1-1')
GO
